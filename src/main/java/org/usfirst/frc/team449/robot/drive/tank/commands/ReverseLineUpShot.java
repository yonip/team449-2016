...
public class ReverseLineUpShot extends CommandGroup {
	...
	/**
	 * This method backs up to a defense after shooting, to avoid spending time
	 * doing so in teleop.
	 */
	public ReverseLineUpShot(StartingPosition startingPosition) {

		int position = startingPosition.toInt();

		// TODO rotate 180 degrees depending on robot orientation

		double distanceFromTop = 4.425 * (position + 0.5); // Distance in feet
															// of center of
															// defense to left
															// wall
		...
		if (position < 0) {
			...
			addSequential(
					new DriveDistance(-(secantVal * (RobotMap.VERTICAL_DISTANCE_TO_LEFT_GOAL - distanceFromTop))));
			...
			double angle = Math.atan((RobotMap.VERTICAL_DISTANCE_TO_FRONT_GOAL - distanceFromTop)
					/ (distanceToGoal - RobotMap.DEFENSE_RAMP_LENGTH - 1)); // -1
																			// because
																			// the
																			// you
																			// want
