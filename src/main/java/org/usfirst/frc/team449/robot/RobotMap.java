...
public abstract class RobotMap {
    ...
    public static abstract class MapObject {
         ...
         * @param objPath the path to find this object in the JSONObject
         */
     ...
     * @throws org.usfirst.frc.team449.robot.RobotMap.ParserException when the requested value doesn't exist
     */
    private static int strictGetInt(String path, JSONObject obj) throws ParserException {
        ...
        for (int i = 0; i < split.length - 1; i++) {
            ...
            if (split[i].matches("\\w+(\\[\\])*(\\[\\d+\\])+$")) { // so like "arrayname[index]" or "arrayname[2nd index][index]"
                String arrGet = split[i];
                Stack<Integer> indices = new Stack<>();
                while (arrGet.matches("\\w+(\\[\\])*(\\[\\d+\\])+$")) {
                    indices.push(Integer.parseInt(arrGet.substring(arrGet.lastIndexOf('[') + 1, arrGet.lastIndexOf(']'))));
    ...
    private static class FatalParserException extends RuntimeException {
        ...
        /**
         * Constructs a FatalParserException with an explanatory message and cause.
         *
         * @param message Detail about the reason for the exception.
         * @param cause   The cause.
         */
