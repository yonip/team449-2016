...
public abstract class RobotMap {
	...
	protected static int getInt(String path, JSONObject obj) {
		...
		// we can collapse the path around anything that islike .instances.<>
		String[] split = path.split("\\.instances\\.\\w+");

		String[] splitters = new String[split.length - 1];

		// find what the <> is
		Pattern p = Pattern.compile("\\.instances\\.\\w+");
		...
		/*
		 * bits of the mask show what slots in the array are used. eg 11 means
		 * use last two, 110 means us 2nd and 3rd to last
		 */
		int mask = (1 << count) - 1;

		// go from the end of the array back, in order of precedence
		String test;
		int temp = -1;
		for (; mask > -1; mask--) {
			test = split[0];
			for (int i = 0; i < count; i++) {
				if (((1 << (count - 1 - i)) & mask) != 0) {
					test += splitters[i];
				}
				test += split[i + 1];
			}
	...
	protected static double getDouble(String path, JSONObject obj) {
		...
		// we can collapse the path around anything that is like .instances.<>
		String[] split = path.split("\\.instances\\.\\w+");
	...
	private static int strictGetInt(String path, JSONObject obj) {
		...
		for (int i = 0; i < split.length - 1; i++) {
			/*
			 * only JSONObjects here; JSONArrays should be done and anything
			 * else is useless
			 */
			...
			if (split[i].matches("\\w+\\[\\d\\]")) { // arrayname[index]
				arr = obj.optJSONArray(split[i].substring(0, split[i].indexOf('[')));
				if (arr == null) {
					return -2;
				}

				// get whatever object is there, if any
				temp = arr.opt(Integer.parseInt(split[i].substring(split[i].indexOf('[') + 1, split[i].indexOf(']'))));
		...
		// if I am here and it's an array, I am just looking for length, right?
		if (temp instanceof JSONArray) {
			if (split[split.length - 1].equals("length")) {
				return ((JSONArray) temp).length();
			}
		}
		// not an array. maybe JSONObject?
		if (temp instanceof JSONObject) {
			obj = (JSONObject) temp;

			// does the target value exist?
			if (!obj.has(split[split.length - 1])) {
				return -4;
			}

			// it exists: get the int or -1 if it is not there
			return obj.optInt(split[split.length - 1], -4);
		}
	...
	private static double strictGetDouble(String path, JSONObject obj) {
		...
		if (temp instanceof JSONObject) {
			...
			// it exists: get the int or -1 if it is not there
			return obj.optDouble(split[split.length - 1], -4);
		}
	...
	private static boolean strictGetBoolean(String path, JSONObject obj) {
		...
		if (temp instanceof JSONObject) {
			...
			// it exists: get the int or -1 if it is not there
			return obj.optBoolean(split[split.length - 1], false);
		}
